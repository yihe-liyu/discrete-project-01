## MarisaLaserFade（Track A / S4c-4）—— 魔理沙非 focus 激光「整批渐隐」控制器。
...
func process(delta: float) -> void:
	...
	if _fading:
		_fade_t -= delta
		sys.set_render_fade(BulletType.Kind.LASER, clampf(_fade_t / FADE_TIME, 0.0, 1.0))
		if _fade_t <= 0.0:
			_fading = false
			_spawned = false
			_despawn_lasers(sys)
...
func _despawn_lasers(sys) -> void:
	for i in range(sys.get_active_count() - 1, -1, -1):
		var bt = sys.get_type(i)
