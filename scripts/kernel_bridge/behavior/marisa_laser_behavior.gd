## MarisaLaserBehavior—— 魔理沙非 focus 激光段的锚定漂移。
...
func process(system: BulletSystem, bullet_id: int, ctx: BehaviorContext) -> void:
	...
	if not (behavior_state is Dictionary):
		behavior_state = {&"drift": params.get(&"initial_drift", 0.0)}
	...
	if not fresh:
		behavior_state[&"drift"] += params.get(&"drift_speed", 2000.0) * system.get_delta()
	system.set_position(bullet_id, anchor + dir * behavior_state[&"drift"])
