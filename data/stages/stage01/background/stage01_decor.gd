...
@onready var ground: BackgroundPlane = stage_background.get_node("Ground") as BackgroundPlane
@onready var sun: BackgroundSun = stage_background.get_node("Sun") as BackgroundSun
@onready var fog: ScreenFogFX = stage_background.get_node("FogFX") as ScreenFogFX
...
func _ready() -> void:
	...
	stage_background.apply_env_preset(ENV_PRESET)
...
func start(p_ctx: StageContext, p_target: Node2D = null):
	...
	# ② 相机移动 + 旋转 (6s)
	timeline.at(6.0).do(func():
		stage_background.pan_camera(Vector3(0, 20, -3), 8.0, Tween.EASE_IN_OUT, Tween.TRANS_QUAD)
		stage_background.rotate_camera(Vector3(deg_to_rad(-30), 0, 0), 6.0, Tween.EASE_IN_OUT, Tween.TRANS_SINE)
	)

	# ③ 地面加速 (10s)
	timeline.at(10.0).do(func():
		var t := stage_background.create_tween().set_process_mode(Tween.TWEEN_PROCESS_PHYSICS)
	...
	# ④ 每 4 帧喷一棵树（持续）
	timeline.at(0.0).every(4.0 / Engine.physics_ticks_per_second).do(func():
	...
	timeline.at(60.0).do(func():
		...
		stage_background.rotate_camera(Vector3(deg_to_rad(-22), 0, 0), 25.0, Tween.EASE_IN_OUT, Tween.TRANS_SINE)
	)
