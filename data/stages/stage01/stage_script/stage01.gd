...
func start(p_ctx: StageContext, p_target: Node2D = null):
	...
	# 0s: BGM
	timeline.at(0.0).do(func(): _stage_director.bgm("stage1"))
	...
	for i in 9:
		timeline.at(52.0 + i).do(func():
			if not ctx.boss.exists():
				_spawn_mid_enemy(0, i, true)
		)
	for i in 7:
		timeline.at(54.5 + i).do(func():
			if not ctx.boss.exists():
				_spawn_mid_enemy(1, i, true)
		)

	for i in 18:
		timeline.at(63.0 + i * 0.7).do(func():
			_spawn_mid_enemy(0, i, false, 1)
		)
	for i in 18:
		timeline.at(63.3 + i * 0.7).do(func():
	...
	for i in 18:
		timeline.at(80.0 + i * 0.3).do(func():
			_spawn_mid_enemy(0, i, false, 1)
		)
	for i in 18:
		timeline.at(80.0 + i * 0.3).do(func():
...
func _exit_tree() -> void:
	if _stage_director:
		_stage_director.dispose()   # 断 dialogue_event 连接 + 清空本关命名槽位
