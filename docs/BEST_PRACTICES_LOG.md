...
- **修复（同波，试玩发现）**：自机不能射击 / 子机消失——`Player._init_shoot_script` 自建一个**无 `stage`** 的 `StageContext`，而 `ctx.refs` 只从 `stage` 取 → 该 ctx 解析不到自机，`ctx.player.get_player()` 恒 null。加 `EntityRegistry.current`（当前世界 `static var` 回退，R8）+ `StageContext.refs` 优先 `stage`、否则回退 `current`。教训：注入型上下文要把「无 stage 的共享 ctx（射击 / 子弹共享 ctx）」纳进解析链，否则静默失效。
