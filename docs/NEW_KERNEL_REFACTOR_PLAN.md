...
- **行数不是判据（2026-09-13 实测更正）**：原「`scripts/kernel_bridge/` ≤ **300** 行」不可达 —— 本判据**明确要求保留**的宿主耦合实测就 ≈ **688 行**（行为桥 273 / bomb 宿主节点 160 / 碰撞·擦弹 165 / laser fade 53 / 延后队列 37），加适配器 236 = **924 行**。判据改为**结构性**（见下），行数只作观测。
- `grep -rn '_by_index' scripts/kernel_bridge/` = **0**（类型 / 端口 / 伤害侧表）；**例外**：`_texture_by_index` 是 M3 ③ 判定保留的**渲染插座**，由 N4 换原生实例缓冲，不计入「侧表债务」。
- `grep -rn 'func kernel_port'` = 0（或仅剩 VM 预留）——**现余 6 处**，是 `_port_by_sig` 行为绑定机制的内容侧、**VM（N3）的前身**；宿主期无法再删。
