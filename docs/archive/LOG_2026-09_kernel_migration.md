# 内核迁移期日志（2026-09-13 ~ 2026-09-16）

> 从 `docs/BEST_PRACTICES_LOG.md` 归档而来（2026-09-19 整理）。
> 这段记录的是 L1–L3.5 / N2 内核迁移，含已废止的「重建版 → tag → vendor」流程。
> **反复踩的坑 / 铁律已蒸馏进 `docs/BEST_PRACTICES_BASELINE.md`**；这里只作可检索的历史底账。
> 排序：新 → 旧。
...
- **§13「现在的建议（不现在开工，但先对齐）」**：整节前提（"不现在开工"）已不成立 —— 4 条建议（先做 M2 / program schema 草案 / 便宜内优化 / 到触发条件再 spike）**全部过期，整节删除**。
- **验证**：`./tools/verify.sh` 全绿 **382 / 4196**（纯文档改动）。
